...
'''
每一行作为一个元素，组成一个列表
每一个元素根据空格（一个制表符），分成若干子元素
扩展名可以通过下角标获取，在'.' 这个符号后面
文件大小是小列表的第二列
因为总行数是固定的，所以，用for 循环读取每一个小列表
进行判断，这个扩展名是否与之前的重复，如果重复，将文件大小相加，如果不重复，重开一个统计行
...
while j <= len(linelist) -1:
    line = linelist[j]
    colunmlist = line.split('\t')
    fileName = colunmlist[0]
    fileFormat = fileName[fileName.find('.')+1:]
    fileSize = colunmlist[1]
    Format_Size.append([fileFormat,fileSize])
    j += 1
#print(Format_Size)
Format = []
x = 0
CalSizeBefore = 0
while x < len(Format_Size):
    if Format_Size[x][0] in Format:
        CalSize = int(Format_Size[x][1]) + CalSizeBefore
    else:
        Format.append(Format_Size[x][0])
    x += 1
print(log)